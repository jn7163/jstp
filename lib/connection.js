...
class Connection extends EventEmitter {
  ...
  //   app - string or object, application to connect to as name
  //         or { name, version }
  ...
  handshake(app, login, password, callback) {
    ...
    const packet = login && password ?
      this._createPacket('handshake', target, 'login', [login, password]) :
      this._createPacket('handshake', target);
  ...
  //   kind - packet kind
  //   target - value or array (will be flattened)
  //            arguments for kind key, usually a name
  ...
  _createPacket(kind, target, verb, args) {
    ...
    if (Array.isArray(target)) {
      packet[kind] = [this._nextPacketId, ...target];
    } else if (target) {
      packet[kind] = [this._nextPacketId, target];
    } else {
      packet[kind] = [this._nextPacketId];
    }
  ...
  _processHandshakeRequest(packet, keys) {
    ...
    const application =
      this.server._getApplication(applicationName, applicationVersion);
